...
#include "openmm/internal/MPIDGeneralizedKirkwoodForceImpl.h"
#include "openmm/internal/MPIDMultipoleForceImpl.h"
#include "openmm/internal/MPIDWcaDispersionForceImpl.h"
#include "openmm/internal/MPIDTorsionTorsionForceImpl.h"
#include "openmm/internal/MPIDVdwForceImpl.h"
...
class CudaCalcMPIDBondForceKernel::ForceInfo : public CudaForceInfo {
public:
    ForceInfo(const MPIDBondForce& force) : force(force) {
    }
...
private:
    const MPIDBondForce& force;
};

CudaCalcMPIDBondForceKernel::CudaCalcMPIDBondForceKernel(std::string name, const Platform& platform, CudaContext& cu, const System& system) : 
                CalcMPIDBondForceKernel(name, platform), cu(cu), system(system), params(NULL) {
}

CudaCalcMPIDBondForceKernel::~CudaCalcMPIDBondForceKernel() {
    cu.setAsCurrent();
    if (params != NULL)
        delete params;
}

void CudaCalcMPIDBondForceKernel::initialize(const System& system, const MPIDBondForce& force) {
    cu.setAsCurrent();
    ...
    replacements["COMPUTE_FORCE"] = CudaMPIDKernelSources::mpidBondForce;
    replacements["PARAMS"] = cu.getBondedUtilities().addArgument(params->getDevicePointer(), "float2");
    replacements["CUBIC_K"] = cu.doubleToString(force.getMPIDGlobalBondCubic());
    replacements["QUARTIC_K"] = cu.doubleToString(force.getMPIDGlobalBondQuartic());
...
double CudaCalcMPIDBondForceKernel::execute(ContextImpl& context, bool includeForces, bool includeEnergy) {
    return 0.0;
}

void CudaCalcMPIDBondForceKernel::copyParametersToContext(ContextImpl& context, const MPIDBondForce& force) {
    cu.setAsCurrent();
    int numContexts = cu.getPlatformData().contexts.size();
    int startIndex = cu.getContextIndex()*force.getNumBonds()/numContexts;
    int endIndex = (cu.getContextIndex()+1)*force.getNumBonds()/numContexts;
...
class CudaCalcMPIDAngleForceKernel::ForceInfo : public CudaForceInfo {
public:
    ForceInfo(const MPIDAngleForce& force) : force(force) {
    }
    int getNumParticleGroups() {
...
private:
    const MPIDAngleForce& force;
};

CudaCalcMPIDAngleForceKernel::CudaCalcMPIDAngleForceKernel(std::string name, const Platform& platform, CudaContext& cu, const System& system) :
            CalcMPIDAngleForceKernel(name, platform), cu(cu), system(system), params(NULL) {
}

CudaCalcMPIDAngleForceKernel::~CudaCalcMPIDAngleForceKernel() {
    cu.setAsCurrent();
    if (params != NULL)
        delete params;
}

void CudaCalcMPIDAngleForceKernel::initialize(const System& system, const MPIDAngleForce& force) {
    cu.setAsCurrent();
    int numContexts = cu.getPlatformData().contexts.size();
    ...
    map<string, string> replacements;
    replacements["APPLY_PERIODIC"] = (force.usesPeriodicBoundaryConditions() ? "1" : "0");
    replacements["COMPUTE_FORCE"] = CudaMPIDKernelSources::mpidAngleForce;
    replacements["PARAMS"] = cu.getBondedUtilities().addArgument(params->getDevicePointer(), "float2");
    replacements["CUBIC_K"] = cu.doubleToString(force.getMPIDGlobalAngleCubic());
    replacements["QUARTIC_K"] = cu.doubleToString(force.getMPIDGlobalAngleQuartic());
    replacements["PENTIC_K"] = cu.doubleToString(force.getMPIDGlobalAnglePentic());
    replacements["SEXTIC_K"] = cu.doubleToString(force.getMPIDGlobalAngleSextic());
...
double CudaCalcMPIDAngleForceKernel::execute(ContextImpl& context, bool includeForces, bool includeEnergy) {
    return 0.0;
}

void CudaCalcMPIDAngleForceKernel::copyParametersToContext(ContextImpl& context, const MPIDAngleForce& force) {
    cu.setAsCurrent();
    int numContexts = cu.getPlatformData().contexts.size();
    int startIndex = cu.getContextIndex()*force.getNumAngles()/numContexts;
    int endIndex = (cu.getContextIndex()+1)*force.getNumAngles()/numContexts;
...
/* -------------------------------------------------------------------------- *
 *                            MPIDInPlaneAngleForce                         *
 * -------------------------------------------------------------------------- */

class CudaCalcMPIDInPlaneAngleForceKernel::ForceInfo : public CudaForceInfo {
public:
    ForceInfo(const MPIDInPlaneAngleForce& force) : force(force) {
    }
    int getNumParticleGroups() {
        return force.getNumAngles();
    }
...
private:
    const MPIDInPlaneAngleForce& force;
};

CudaCalcMPIDInPlaneAngleForceKernel::CudaCalcMPIDInPlaneAngleForceKernel(std::string name, const Platform& platform, CudaContext& cu, const System& system) : 
          CalcMPIDInPlaneAngleForceKernel(name, platform), cu(cu), system(system), params(NULL) {
}

CudaCalcMPIDInPlaneAngleForceKernel::~CudaCalcMPIDInPlaneAngleForceKernel() {
    cu.setAsCurrent();
    if (params != NULL)
        delete params;
}

void CudaCalcMPIDInPlaneAngleForceKernel::initialize(const System& system, const MPIDInPlaneAngleForce& force) {
    cu.setAsCurrent();
    int numContexts = cu.getPlatformData().contexts.size();
    int startIndex = cu.getContextIndex()*force.getNumAngles()/numContexts;
    int endIndex = (cu.getContextIndex()+1)*force.getNumAngles()/numContexts;
    ...
    replacements["APPLY_PERIODIC"] = (force.usesPeriodicBoundaryConditions() ? "1" : "0");
    replacements["PARAMS"] = cu.getBondedUtilities().addArgument(params->getDevicePointer(), "float2");
    replacements["CUBIC_K"] = cu.doubleToString(force.getMPIDGlobalInPlaneAngleCubic());
    replacements["QUARTIC_K"] = cu.doubleToString(force.getMPIDGlobalInPlaneAngleQuartic());
    replacements["PENTIC_K"] = cu.doubleToString(force.getMPIDGlobalInPlaneAnglePentic());
    replacements["SEXTIC_K"] = cu.doubleToString(force.getMPIDGlobalInPlaneAngleSextic());
    replacements["RAD_TO_DEG"] = cu.doubleToString(180/M_PI);
    cu.getBondedUtilities().addInteraction(atoms, cu.replaceStrings(CudaMPIDKernelSources::mpidInPlaneForce, replacements), force.getForceGroup());
    cu.addForce(new ForceInfo(force));
}

double CudaCalcMPIDInPlaneAngleForceKernel::execute(ContextImpl& context, bool includeForces, bool includeEnergy) {
    return 0.0;
}

void CudaCalcMPIDInPlaneAngleForceKernel::copyParametersToContext(ContextImpl& context, const MPIDInPlaneAngleForce& force) {
    cu.setAsCurrent();
    int numContexts = cu.getPlatformData().contexts.size();
    int startIndex = cu.getContextIndex()*force.getNumAngles()/numContexts;
    int endIndex = (cu.getContextIndex()+1)*force.getNumAngles()/numContexts;
...
class CudaCalcMPIDPiTorsionForceKernel::ForceInfo : public CudaForceInfo {
public:
    ForceInfo(const MPIDPiTorsionForce& force) : force(force) {
    }
    int getNumParticleGroups() {
...
private:
    const MPIDPiTorsionForce& force;
};

CudaCalcMPIDPiTorsionForceKernel::CudaCalcMPIDPiTorsionForceKernel(std::string name, const Platform& platform, CudaContext& cu, const System& system) :
         CalcMPIDPiTorsionForceKernel(name, platform), cu(cu), system(system), params(NULL) {
}

CudaCalcMPIDPiTorsionForceKernel::~CudaCalcMPIDPiTorsionForceKernel() {
    cu.setAsCurrent();
    if (params != NULL)
        delete params;
}

void CudaCalcMPIDPiTorsionForceKernel::initialize(const System& system, const MPIDPiTorsionForce& force) {
    cu.setAsCurrent();
    int numContexts = cu.getPlatformData().contexts.size();
    ...
    cu.getBondedUtilities().addInteraction(atoms, cu.replaceStrings(CudaMPIDKernelSources::mpidPiTorsionForce, replacements), force.getForceGroup());
    cu.addForce(new ForceInfo(force));
}

double CudaCalcMPIDPiTorsionForceKernel::execute(ContextImpl& context, bool includeForces, bool includeEnergy) {
    return 0.0;
}

void CudaCalcMPIDPiTorsionForceKernel::copyParametersToContext(ContextImpl& context, const MPIDPiTorsionForce& force) {
    cu.setAsCurrent();
    int numContexts = cu.getPlatformData().contexts.size();
    int startIndex = cu.getContextIndex()*force.getNumPiTorsions()/numContexts;
    int endIndex = (cu.getContextIndex()+1)*force.getNumPiTorsions()/numContexts;
...
class CudaCalcMPIDStretchBendForceKernel::ForceInfo : public CudaForceInfo {
public:
    ForceInfo(const MPIDStretchBendForce& force) : force(force) {
    }
    int getNumParticleGroups() {
...
private:
    const MPIDStretchBendForce& force;
};

CudaCalcMPIDStretchBendForceKernel::CudaCalcMPIDStretchBendForceKernel(std::string name, const Platform& platform, CudaContext& cu, const System& system) :
                   CalcMPIDStretchBendForceKernel(name, platform), cu(cu), system(system), params1(NULL), params2(NULL) {
}

CudaCalcMPIDStretchBendForceKernel::~CudaCalcMPIDStretchBendForceKernel() {
    cu.setAsCurrent();
    if (params1 != NULL)
        delete params1;
    if (params2 != NULL)
...
void CudaCalcMPIDStretchBendForceKernel::initialize(const System& system, const MPIDStretchBendForce& force) {
    cu.setAsCurrent();
    int numContexts = cu.getPlatformData().contexts.size();
    ...
    replacements["RAD_TO_DEG"] = cu.doubleToString(180/M_PI);
    cu.getBondedUtilities().addInteraction(atoms, cu.replaceStrings(CudaMPIDKernelSources::mpidStretchBendForce, replacements), force.getForceGroup());
    cu.addForce(new ForceInfo(force));
}

double CudaCalcMPIDStretchBendForceKernel::execute(ContextImpl& context, bool includeForces, bool includeEnergy) {
    return 0.0;
}

void CudaCalcMPIDStretchBendForceKernel::copyParametersToContext(ContextImpl& context, const MPIDStretchBendForce& force) {
    cu.setAsCurrent();
    int numContexts = cu.getPlatformData().contexts.size();
    int startIndex = cu.getContextIndex()*force.getNumStretchBends()/numContexts;
    int endIndex = (cu.getContextIndex()+1)*force.getNumStretchBends()/numContexts;
...
class CudaCalcMPIDOutOfPlaneBendForceKernel::ForceInfo : public CudaForceInfo {
public:
    ForceInfo(const MPIDOutOfPlaneBendForce& force) : force(force) {
    }
    int getNumParticleGroups() {
...
private:
    const MPIDOutOfPlaneBendForce& force;
};

CudaCalcMPIDOutOfPlaneBendForceKernel::CudaCalcMPIDOutOfPlaneBendForceKernel(std::string name, const Platform& platform, CudaContext& cu, const System& system) :
          CalcMPIDOutOfPlaneBendForceKernel(name, platform), cu(cu), system(system), params(NULL) {
}

CudaCalcMPIDOutOfPlaneBendForceKernel::~CudaCalcMPIDOutOfPlaneBendForceKernel() {
    cu.setAsCurrent();
    if (params != NULL)
        delete params;
}

void CudaCalcMPIDOutOfPlaneBendForceKernel::initialize(const System& system, const MPIDOutOfPlaneBendForce& force) {
    cu.setAsCurrent();
    int numContexts = cu.getPlatformData().contexts.size();
    ...
    replacements["APPLY_PERIODIC"] = (force.usesPeriodicBoundaryConditions() ? "1" : "0");
    replacements["PARAMS"] = cu.getBondedUtilities().addArgument(params->getDevicePointer(), "float");
    replacements["CUBIC_K"] = cu.doubleToString(force.getMPIDGlobalOutOfPlaneBendCubic());
    replacements["QUARTIC_K"] = cu.doubleToString(force.getMPIDGlobalOutOfPlaneBendQuartic());
    replacements["PENTIC_K"] = cu.doubleToString(force.getMPIDGlobalOutOfPlaneBendPentic());
    replacements["SEXTIC_K"] = cu.doubleToString(force.getMPIDGlobalOutOfPlaneBendSextic());
    replacements["RAD_TO_DEG"] = cu.doubleToString(180/M_PI);
    cu.getBondedUtilities().addInteraction(atoms, cu.replaceStrings(CudaMPIDKernelSources::mpidOutOfPlaneBendForce, replacements), force.getForceGroup());
    cu.addForce(new ForceInfo(force));
}

double CudaCalcMPIDOutOfPlaneBendForceKernel::execute(ContextImpl& context, bool includeForces, bool includeEnergy) {
    return 0.0;
}

void CudaCalcMPIDOutOfPlaneBendForceKernel::copyParametersToContext(ContextImpl& context, const MPIDOutOfPlaneBendForce& force) {
    cu.setAsCurrent();
    int numContexts = cu.getPlatformData().contexts.size();
    int startIndex = cu.getContextIndex()*force.getNumOutOfPlaneBends()/numContexts;
    int endIndex = (cu.getContextIndex()+1)*force.getNumOutOfPlaneBends()/numContexts;
...
class CudaCalcMPIDTorsionTorsionForceKernel::ForceInfo : public CudaForceInfo {
public:
    ForceInfo(const MPIDTorsionTorsionForce& force) : force(force) {
    }
    int getNumParticleGroups() {
...
private:
    const MPIDTorsionTorsionForce& force;
};

CudaCalcMPIDTorsionTorsionForceKernel::CudaCalcMPIDTorsionTorsionForceKernel(std::string name, const Platform& platform, CudaContext& cu, const System& system) :
                CalcMPIDTorsionTorsionForceKernel(name, platform), cu(cu), system(system), gridValues(NULL), gridParams(NULL), torsionParams(NULL) {
}

CudaCalcMPIDTorsionTorsionForceKernel::~CudaCalcMPIDTorsionTorsionForceKernel() {
    cu.setAsCurrent();
    if (gridValues != NULL)
        delete gridValues;
    if (gridParams != NULL)
...
void CudaCalcMPIDTorsionTorsionForceKernel::initialize(const System& system, const MPIDTorsionTorsionForce& force) {
    cu.setAsCurrent();
    int numContexts = cu.getPlatformData().contexts.size();
    ...
    for (int i = 0; i < force.getNumTorsionTorsionGrids(); i++) {
        ...
        if (initialGrid[0][0][0] != initialGrid[0][1][0]) {
            MPIDTorsionTorsionForceImpl::reorderGrid(initialGrid, reorderedGrid);
            reordered = true;
        }
    ...
    replacements["RAD_TO_DEG"] = cu.doubleToString(180/M_PI);
    cu.getBondedUtilities().addInteraction(atoms, cu.replaceStrings(CudaMPIDKernelSources::mpidTorsionTorsionForce, replacements), force.getForceGroup());
    cu.getBondedUtilities().addPrefixCode(CudaMPIDKernelSources::bicubic);
    cu.addForce(new ForceInfo(force));
}

double CudaCalcMPIDTorsionTorsionForceKernel::execute(ContextImpl& context, bool includeForces, bool includeEnergy) {
    return 0.0;
}
...
class CudaCalcMPIDMultipoleForceKernel::ForceInfo : public CudaForceInfo {
public:
    ForceInfo(const MPIDMultipoleForce& force) : force(force) {
    }
    ...
    void getParticlesInGroup(int index, vector<int>& particles) {
        ...
        force.getCovalentMap(particle, MPIDMultipoleForce::CovalentType(type), particles);
    }
    bool areGroupsIdentical(int group1, int group2) {
        return ((group1%7) == (group2%7));
    }
private:
    const MPIDMultipoleForce& force;
};

CudaCalcMPIDMultipoleForceKernel::CudaCalcMPIDMultipoleForceKernel(std::string name, const Platform& platform, CudaContext& cu, const System& system) : 
        CalcMPIDMultipoleForceKernel(name, platform), cu(cu), system(system), hasInitializedScaleFactors(false), hasInitializedFFT(false), multipolesAreValid(false), hasCreatedEvent(false),
...
CudaCalcMPIDMultipoleForceKernel::~CudaCalcMPIDMultipoleForceKernel() {
    cu.setAsCurrent();
...
void CudaCalcMPIDMultipoleForceKernel::initialize(const System& system, const MPIDMultipoleForce& force) {
    cu.setAsCurrent();
    ...
    if (polarizationType == MPIDMultipoleForce::Mutual) {
        ...
        CHECK_RESULT(cuEventCreate(&syncEvent, CU_EVENT_DISABLE_TIMING), "Error creating event for MPIDMultipoleForce");
        hasCreatedEvent = true;
    }
    else if (polarizationType == MPIDMultipoleForce::Extrapolated) {
    ...
    for (int i = 0; i < numMultipoles; i++) {
        ...
        set<int> allAtoms;
        allAtoms.insert(i);
        force.getCovalentMap(i, MPIDMultipoleForce::Covalent12, atoms);
        allAtoms.insert(atoms.begin(), atoms.end());
        force.getCovalentMap(i, MPIDMultipoleForce::Covalent13, atoms);
        allAtoms.insert(atoms.begin(), atoms.end());
        for (int atom : allAtoms)
            covalentFlagValues.push_back(make_int3(i, atom, 0));
        force.getCovalentMap(i, MPIDMultipoleForce::Covalent14, atoms);
        allAtoms.insert(atoms.begin(), atoms.end());
        for (int atom : atoms)
            covalentFlagValues.push_back(make_int3(i, atom, 1));
        force.getCovalentMap(i, MPIDMultipoleForce::Covalent15, atoms);
        for (int atom : atoms)
            covalentFlagValues.push_back(make_int3(i, atom, 2));
        allAtoms.insert(atoms.begin(), atoms.end());
        force.getCovalentMap(i, MPIDMultipoleForce::PolarizationCovalent11, atoms);
        allAtoms.insert(atoms.begin(), atoms.end());
        ...
        vector<int> atoms12;
        force.getCovalentMap(i, MPIDMultipoleForce::PolarizationCovalent12, atoms12);
        for (int atom : atoms)
    ...
    if (polarizationType != MPIDMultipoleForce::Direct) {
    ...
    usePME = (force.getNonbondedMethod() == MPIDMultipoleForce::PME);
    
    // See whether there's an MPIDGeneralizedKirkwoodForce in the System.

    const MPIDGeneralizedKirkwoodForce* gk = NULL;
    for (int i = 0; i < system.getNumForces() && gk == NULL; i++)
        gk = dynamic_cast<const MPIDGeneralizedKirkwoodForce*>(&system.getForce(i));
    ...
    if (polarizationType == MPIDMultipoleForce::Extrapolated)
    ...
    else if (polarizationType == MPIDMultipoleForce::Mutual)
    ...
    CUmodule module = cu.createModule(CudaKernelSources::vectorOps+CudaMPIDKernelSources::multipoles, defines);
    ...
    module = cu.createModule(CudaKernelSources::vectorOps+CudaMPIDKernelSources::multipoleFixedField, defines);
    ...
    if (polarizationType != MPIDMultipoleForce::Direct) {
        ...
        module = cu.createModule(CudaKernelSources::vectorOps+CudaMPIDKernelSources::multipoleInducedField, defines);
    ...
    if (usePME) {
        ...
        CUmodule module = cu.createModule(CudaKernelSources::vectorOps+CudaMPIDKernelSources::multipolePme, pmeDefines);
...
void CudaCalcMPIDMultipoleForceKernel::initializeScaleFactors() {
...
double CudaCalcMPIDMultipoleForceKernel::execute(ContextImpl& context, bool includeForces, bool includeEnergy) {
    if (!hasInitializedScaleFactors) {
        initializeScaleFactors();
        for (auto impl : context.getForceImpls()) {
            MPIDGeneralizedKirkwoodForceImpl* gkImpl = dynamic_cast<MPIDGeneralizedKirkwoodForceImpl*>(impl);
            if (gkImpl != NULL) {
                gkKernel = dynamic_cast<CudaCalcMPIDGeneralizedKirkwoodForceKernel*>(&gkImpl->getKernel().getImpl());
                break;
            }
        }
    }
...
void CudaCalcMPIDMultipoleForceKernel::computeInducedField(void** recipBoxVectorPointer) {
    CudaNonbondedUtilities& nb = cu.getNonbondedUtilities();
    int startTileIndex = nb.getStartTileIndex();
    int numTileIndices = nb.getNumTiles();
    int numForceThreadBlocks = nb.getNumForceThreadBlocks();
...
bool CudaCalcMPIDMultipoleForceKernel::iterateDipolesByDIIS(int iteration) {
    void* npt = NULL;
...
void CudaCalcMPIDMultipoleForceKernel::computeExtrapolatedDipoles(void** recipBoxVectorPointer) {
...
void CudaCalcMPIDMultipoleForceKernel::ensureMultipolesValid(ContextImpl& context) {
...
void CudaCalcMPIDMultipoleForceKernel::getLabFramePermanentDipoles(ContextImpl& context, vector<Vec3>& dipoles) {
...
void CudaCalcMPIDMultipoleForceKernel::getInducedDipoles(ContextImpl& context, vector<Vec3>& dipoles) {
    ensureMultipolesValid(context);
    int numParticles = cu.getNumAtoms();
    dipoles.resize(numParticles);
    const vector<int>& order = cu.getAtomIndex();
...
void CudaCalcMPIDMultipoleForceKernel::getTotalDipoles(ContextImpl& context, vector<Vec3>& dipoles) {
    ensureMultipolesValid(context);
    int numParticles = cu.getNumAtoms();
    dipoles.resize(numParticles);
    const vector<int>& order = cu.getAtomIndex();
...
void CudaCalcMPIDMultipoleForceKernel::getElectrostaticPotential(ContextImpl& context, const vector<Vec3>& inputGrid, vector<double>& outputElectrostaticPotential) {
    ensureMultipolesValid(context);
...
void CudaCalcMPIDMultipoleForceKernel::computeSystemMultipoleMoments(ContextImpl& context, vector<double>& outputMultipoleMoments) {
...
void CudaCalcMPIDMultipoleForceKernel::getSystemMultipoleMoments(ContextImpl& context, vector<double>& outputMultipoleMoments) {
    ensureMultipolesValid(context);
    if (cu.getUseDoublePrecision())
...
void CudaCalcMPIDMultipoleForceKernel::copyParametersToContext(ContextImpl& context, const MPIDMultipoleForce& force) {
...
void CudaCalcMPIDMultipoleForceKernel::getPMEParameters(double& alpha, int& nx, int& ny, int& nz) const {
    if (!usePME)
...
/* -------------------------------------------------------------------------- *
 *                       MPIDGeneralizedKirkwood                            *
 * -------------------------------------------------------------------------- */

class CudaCalcMPIDGeneralizedKirkwoodForceKernel::ForceInfo : public CudaForceInfo {
public:
    ForceInfo(const MPIDGeneralizedKirkwoodForce& force) : force(force) {
    }
    bool areParticlesIdentical(int particle1, int particle2) {
...
private:
    const MPIDGeneralizedKirkwoodForce& force;
};

CudaCalcMPIDGeneralizedKirkwoodForceKernel::CudaCalcMPIDGeneralizedKirkwoodForceKernel(std::string name, const Platform& platform, CudaContext& cu, const System& system) : 
           CalcMPIDGeneralizedKirkwoodForceKernel(name, platform), cu(cu), system(system), hasInitializedKernels(false), params(NULL), bornRadii(NULL), field(NULL),
           inducedField(NULL), inducedFieldPolar(NULL), inducedDipoleS(NULL), inducedDipolePolarS(NULL), bornSum(NULL), bornForce(NULL) {
}

CudaCalcMPIDGeneralizedKirkwoodForceKernel::~CudaCalcMPIDGeneralizedKirkwoodForceKernel() {
    cu.setAsCurrent();
    if (params != NULL)
        delete params;
    if (bornRadii != NULL)
...
void CudaCalcMPIDGeneralizedKirkwoodForceKernel::initialize(const System& system, const MPIDGeneralizedKirkwoodForce& force) {
    cu.setAsCurrent();
    if (cu.getPlatformData().contexts.size() > 1)
        throw OpenMMException("MPIDGeneralizedKirkwoodForce does not support using multiple CUDA devices");
    const MPIDMultipoleForce* multipoles = NULL;
    for (int i = 0; i < system.getNumForces() && multipoles == NULL; i++)
        multipoles = dynamic_cast<const MPIDMultipoleForce*>(&system.getForce(i));
    if (multipoles == NULL)
        throw OpenMMException("MPIDGeneralizedKirkwoodForce requires the System to also contain an MPIDMultipoleForce");
    CudaNonbondedUtilities& nb = cu.getNonbondedUtilities();
    int paddedNumAtoms = cu.getPaddedNumAtoms();
    int elementSize = (cu.getUseDoublePrecision() ? sizeof(double) : sizeof(float));
    params = CudaArray::create<float2>(cu, paddedNumAtoms, "mpidGkParams");
    ...
    for (int i = 0; i < force.getNumParticles(); i++) {
        ...
        // Make sure the charge matches the one specified by the MPIDMultipoleForce.
        ...
        if (charge != charge2)
            throw OpenMMException("MPIDGeneralizedKirkwoodForce and MPIDMultipoleForce must specify the same charge for every atom");
    }
...
double CudaCalcMPIDGeneralizedKirkwoodForceKernel::execute(ContextImpl& context, bool includeForces, bool includeEnergy) {
    // Since GK is so tightly entwined with the electrostatics, this method does nothing, and the force calculation
    // is driven by MPIDMultipoleForce.
    return 0.0;
}

void CudaCalcMPIDGeneralizedKirkwoodForceKernel::computeBornRadii() {
    if (!hasInitializedKernels) {
        ...
        forceSource << CudaMPIDKernelSources::mpidGk;
        forceSource << "#define F1\n";
        forceSource << CudaMPIDKernelSources::gkPairForce1;
        forceSource << CudaMPIDKernelSources::gkPairForce2;
        forceSource << CudaMPIDKernelSources::gkEDiffPairForce;
...
void CudaCalcMPIDGeneralizedKirkwoodForceKernel::finishComputation(CudaArray& torque, CudaArray& labFrameDipoles, CudaArray& labFrameQuadrupoles,
...
void CudaCalcMPIDGeneralizedKirkwoodForceKernel::copyParametersToContext(ContextImpl& context, const MPIDGeneralizedKirkwoodForce& force) {
    // Make sure the new parameters are acceptable.
...
class CudaCalcMPIDVdwForceKernel::ForceInfo : public CudaForceInfo {
public:
    ForceInfo(const MPIDVdwForce& force) : force(force) {
    }
    bool areParticlesIdentical(int particle1, int particle2) {
        int iv1, iv2;
...
private:
    const MPIDVdwForce& force;
};

CudaCalcMPIDVdwForceKernel::CudaCalcMPIDVdwForceKernel(std::string name, const Platform& platform, CudaContext& cu, const System& system) :
        CalcMPIDVdwForceKernel(name, platform), cu(cu), system(system), hasInitializedNonbonded(false), sigmaEpsilon(NULL),
        bondReductionAtoms(NULL), bondReductionFactors(NULL), tempPosq(NULL), tempForces(NULL), nonbonded(NULL) {
}

CudaCalcMPIDVdwForceKernel::~CudaCalcMPIDVdwForceKernel() {
    cu.setAsCurrent();
...
void CudaCalcMPIDVdwForceKernel::initialize(const System& system, const MPIDVdwForce& force) {
    cu.setAsCurrent();
    ...
    if (force.getUseDispersionCorrection())
        dispersionCoefficient = MPIDVdwForceImpl::calcDispersionCorrection(system, force);
    else
    ...
    bool useCutoff = (force.getNonbondedMethod() != MPIDVdwForce::NoCutoff);
    nonbonded->addInteraction(useCutoff, useCutoff, true, force.getCutoffDistance(), exclusions,
        cu.replaceStrings(CudaMPIDKernelSources::mpidVdwForce2, replacements), 0);
    
    // Create the other kernels.
    
    map<string, string> defines;
    defines["PADDED_NUM_ATOMS"] = cu.intToString(cu.getPaddedNumAtoms());
    CUmodule module = cu.createModule(CudaMPIDKernelSources::mpidVdwForce1, defines);
    prepareKernel = cu.getKernel(module, "prepareToComputeForce");
    spreadKernel = cu.getKernel(module, "spreadForces");
    cu.addForce(new ForceInfo(force));
}

double CudaCalcMPIDVdwForceKernel::execute(ContextImpl& context, bool includeForces, bool includeEnergy) {
...
void CudaCalcMPIDVdwForceKernel::copyParametersToContext(ContextImpl& context, const MPIDVdwForce& force) {
    // Make sure the new parameters are acceptable.
...
class CudaCalcMPIDWcaDispersionForceKernel::ForceInfo : public CudaForceInfo {
public:
    ForceInfo(const MPIDWcaDispersionForce& force) : force(force) {
    }
    bool areParticlesIdentical(int particle1, int particle2) {
...
private:
    const MPIDWcaDispersionForce& force;
};

CudaCalcMPIDWcaDispersionForceKernel::CudaCalcMPIDWcaDispersionForceKernel(std::string name, const Platform& platform, CudaContext& cu, const System& system) : 
           CalcMPIDWcaDispersionForceKernel(name, platform), cu(cu), system(system), radiusEpsilon(NULL) {
}

CudaCalcMPIDWcaDispersionForceKernel::~CudaCalcMPIDWcaDispersionForceKernel() {
    cu.setAsCurrent();
    if (radiusEpsilon != NULL)
        delete radiusEpsilon;
}

void CudaCalcMPIDWcaDispersionForceKernel::initialize(const System& system, const MPIDWcaDispersionForce& force) {
    ...
    defines["M_PI"] = cu.doubleToString(M_PI);
    CUmodule module = cu.createModule(CudaKernelSources::vectorOps+CudaMPIDKernelSources::mpidWcaForce, defines);
    forceKernel = cu.getKernel(module, "computeWCAForce");
    totalMaximumDispersionEnergy = MPIDWcaDispersionForceImpl::getTotalMaximumDispersionEnergy(force);
...
double CudaCalcMPIDWcaDispersionForceKernel::execute(ContextImpl& context, bool includeForces, bool includeEnergy) {
    CudaNonbondedUtilities& nb = cu.getNonbondedUtilities();
    int startTileIndex = nb.getStartTileIndex();
    int numTileIndices = nb.getNumTiles();
    int numForceThreadBlocks = nb.getNumForceThreadBlocks();
...
void CudaCalcMPIDWcaDispersionForceKernel::copyParametersToContext(ContextImpl& context, const MPIDWcaDispersionForce& force) {
    // Make sure the new parameters are acceptable.
